...
tf.summary.scalar('train_loss', 0)
tf.summary.scalar('valid_loss', 0)
# Dataset directory
...
def get_default_hparams():
    ...
    hparams = tf.contrib.training.HParams(
        categories=['pig','bee','flower','bus','giraffe'],  
        ...
        max_stroke_len = -1,
        max_strokes_num = -1,
        z_size=64,  # Size of latent variable
        ...
        random_scale_factor=0.,  # Random scaling data augmention proportion
        augment_stroke_prob=0.,  # Point dropping augmentation proportion
...
def _train(sess, model, train_set, epoch, sum):
    ...
    for begin, end in zip(range(0, len(index), model.hps.batch_size),
        ...
        seqs, pngs, labels, seq_len,s_n,stroke_len,stroke_num,_,box,_  = train_set._get_batch_from_indices(batch_index)
    ...
    #tf.summary.scalar('train_loss', loss/train_set.num_batches)
    epoch += 1
    return epoch, sum
...
def train_model(model_params):
    ...
    for epo in range(100000):
        ...
        if (epoch % model_params.save_every) == 0 :
            ...
            #tf.summary.scalar('valid_loss', valid_cost)
