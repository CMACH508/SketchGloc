...
def sample(sess, sample_model, z, gen_size=1, seq_len=250, temperature=0.24, greedy_mode=True):
...
def get_sam_p(sess,test_set,t_model_params,index):
    ...
    num_per_category=  t_model_params.batch_size
    conditional = True
...
def sample_def(sess,test_set,t_model_params,index,model,draw_model):
    ...
    output_dir = './sample_ds3/sample_ds3'

    # Number of generated samples per category
    num_per_category= 300
    conditional = True
...
def main():
    ...
    utils.load_checkpoint(sess, model_dir,ckpt_id)
    cnt_name = -1
